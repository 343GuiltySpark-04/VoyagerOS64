...
/* Limine requests are defined ONCE in a separate TU (e.g., boot/limine_requests.c).
   Here we only extern them. */
...
void frame_init(void)
{
    ...
    uint64_t kphys_begin = kphys_base + ((uint64_t)__kernel_start - kvirt_base);
    ...
    if (module_request.response)
    {
        for (size_t i = 0; i < module_request.response->module_count; i++)
        {
            ...
            mark_range_allocated((paddr_t)m->address, (paddr_t)(m->address + m->size));
        }
    }
...
void n_frame_free(paddr_t paddr)
{
    ...
    {
        spinlock_release(&g_memlock);
        panic("frame_free: non-usable paddr=0x%llx", (unsigned long long)paddr);
    }
    if (!bm_test(g_alloc_bm, i))
    {
        spinlock_release(&g_memlock);
        panic("frame_free: double-free paddr=0x%llx", (unsigned long long)paddr);
    }
