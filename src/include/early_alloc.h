...
void    early_reserved_region(paddr_t *base,
                              size_t  *pages); // report what we consumed
size_t  early_pages_used(void);

// Optional: reserve a specific region for early alloc (must be USABLE and
// page-aligned). If you call this before early_init(), early_init() will be a
// no-op.
