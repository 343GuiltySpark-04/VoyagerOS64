...
#define VECTOR_INSERT(VEC, IDX, VALUE)                           \
    ...
    {                                                            \
        ...
        for (size_t VECTOR_INSERT_i = VECTOR_INSERT_vec->length; \
             VECTOR_INSERT_i > VECTOR_INSERT_index;              \
             VECTOR_INSERT_i--)                                  \
        {                                                        \
            VECTOR_INSERT_vec->data[VECTOR_INSERT_i] =           \
                VECTOR_INSERT_vec->data[VECTOR_INSERT_i - 1];    \
        }                                                        \
...
#define VECTOR_REMOVE(VEC, IDX)                               \
    ...
    {                                                         \
        ...
        for (size_t VECTOR_REMOVE_i = (IDX);                  \
             VECTOR_REMOVE_i < VECTOR_REMOVE_vec->length - 1; \
             VECTOR_REMOVE_i++)                               \
        {                                                     \
            VECTOR_REMOVE_vec->data[VECTOR_REMOVE_i] =        \
                VECTOR_REMOVE_vec->data[VECTOR_REMOVE_i + 1]; \
        }                                                     \
...
#define VECTOR_ITEM(VEC, IDX)                                            \
    ({                                                                   \
        ...
        __auto_type VECTOR_ITEM_result =                                 \
            (typeof(*VECTOR_ITEM_vec->data)) VECTOR_INVALID_INDEX;       \
...
#define VECTOR_FIND(VEC, VALUE)                                  \
    ({                                                           \
        ...
        for (size_t VECTOR_FIND_i = 0;                           \
             VECTOR_FIND_i < VECTOR_FIND_vec->length;            \
             VECTOR_FIND_i++)                                    \
        {                                                        \
...
#define VECTOR_REMOVE_BY_VALUE(VEC, VALUE)                                     \
    do                                                                         \
    {                                                                          \
        __auto_type VECTOR_REMOVE_BY_VALUE_vec = VEC;                          \
        __auto_type VECTOR_REMOVE_BY_VALUE_v   = VALUE;                        \
        size_t VECTOR_REMOVE_BY_VALUE_i =                                      \
            VECTOR_FIND(VECTOR_REMOVE_BY_VALUE_vec, VECTOR_REMOVE_BY_VALUE_v); \
...
#define VECTOR_FOR_EACH(VEC, BINDING, ...)                     \
    ...
    {                                                          \
        ...
        for (size_t VECTOR_FOR_EACH_i = 0;                     \
             VECTOR_FOR_EACH_i < VECTOR_FOR_EACH_vec->length;  \
             VECTOR_FOR_EACH_i++)                              \
        {                                                      \
            __auto_type BINDING =                              \
                &VECTOR_FOR_EACH_vec->data[VECTOR_FOR_EACH_i]; \
            __VA_ARGS__                                        \
        }                                                      \
    } while (0)
