...
static void
plot_char(struct term_context *_ctx, struct fbterm_char *c, size_t x, size_t y)
{
    struct fbterm_context *ctx = (void *) _ctx;
...
static void plot_char_fast(struct term_context *_ctx,
...
static void push_to_queue(struct term_context *_ctx,
                          struct fbterm_char  *c,
                          size_t               x,
                          size_t               y)
{
...
static void fbterm_revscroll(struct term_context *_ctx)
{
    ...
    for (size_t i = _ctx->scroll_top_margin * _ctx->cols;
         i < (_ctx->scroll_top_margin + 1) * _ctx->cols;
         i++)
    {
...
static void
fbterm_get_cursor_pos(struct term_context *_ctx, size_t *x, size_t *y)
{
    struct fbterm_context *ctx = (void *) _ctx;
...
static void fbterm_move_character(struct term_context *_ctx,
                                  size_t               new_x,
                                  size_t               new_y,
                                  size_t               old_x,
                                  size_t               old_y)
...
static void fbterm_deinit(struct term_context *_ctx,
                                 ...
                                 size_t    margin)
{
    ...
    {
        memcpy(ctx->ansi_bright_colours,
               ansi_bright_colours,
...
#undef FONT_BYTES
    ...
    ctx->font_bool_size =
        FBTERM_FONT_GLYPHS * font_height * ctx->font_width * sizeof(bool);
    ...
    ctx->queue_size =
        _ctx->rows * _ctx->cols * sizeof(struct fbterm_queue_item);
