...
void read_memory_map()
{
    ...
    largestFreeMemSegmentSize =
        memmap_req.response->entries[bitmap_entry]->length;
...
void *frame_request_multiple(uint32_t count)
{
    ...
    for (uint32_t i = 0; i < bitmapSize * 8; i++)
    {
        if (bitmap_get(frameBitmap, i) == false)
        {
            if (freeCount == count)
            {
                ...
                frame_lock_multiple((void *) ((uint64_t) index * 0x1000),
                                    count);
    ...
    return NULL; // in the future page swap to file, for now just fucking page
                 // fault.
}
